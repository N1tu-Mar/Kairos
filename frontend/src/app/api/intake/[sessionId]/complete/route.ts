...
import { completeIntake } from "@/lib/api";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    return NextResponse.json(await completeIntake(sessionId, body.expected_revision));
  } catch (error) {
    return errorResponse(
      error,
      "POST /api/intake/[sessionId]/complete",
      "The founder profile could not be completed.",
      { 409: "Confirm every required fact before finishing." },
    );
  }
}
